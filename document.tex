...
その要因として，移動ロボットの不整地走破性能における一般的な指標が存在しないことが挙げられる．

%先行研究での課題
先行研究における不整地走破性能の評価方法は，段差の高さや斜度，間隙の長さなどのスケールを用いた指標や
\cite{TITANXX,DFC,six_legged_robot,ro-ba}，
ロボットフィールドや実際の災害現場のような特定の環境内における速度や加速度，走破可否といった走行性能によって検証されてきた
...
このように，移動ロボットの走行性能を周波数領域で評価することで，様々な地形に応じた走行性能を評価できると考えた．
...
\begin{figure}[t]
    ...
    {figure/研究内容のイメージ図.png}
    %重ね合わせによる走行性能の評価イメージ
    \caption{Image of mobility evaluation based on decomposition and superposition}
    \label{fig:研究概要1研究内容のイメージ図}
\end{figure}

\begin{figure}[t]
    \centering
    \includegraphics[width=1.0\linewidth]
    {figure/simのイメージ図.png}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=1.0\linewidth]
    {figure/速度の最大最小差分の周波数応答.png}
    %周波数応答の速度差分のグラフイメージ
    \caption{Relationship between the normalized velocity variation (ΔV/A) and terrain frequency}
    \label{fig:イメージグラフ}
\end{figure}
